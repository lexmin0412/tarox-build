...
function processStyleImports(content, processFn) {
    ...
    content = content.replace(helper_1.CSS_IMPORT_REG, (m, $1, $2) => {
        if (styleReg.test($2)) {
            style.push(m);
            imports.push($2);
            if (processFn) {
...
class Convertor {
    constructor(root) {
        this.wxsIncrementId = helper_2.incrementId();
        this.root = root;
    ...
    initConvert() {
        if (fs.existsSync(this.convertRoot)) {
            helper_1.emptyDirectory(this.convertRoot, { excludes: ['node_modules'] });
        }
        else {
        ...
        babel_traverse_1.default(ast, {
            Program: {
                enter(astPath) {
                    astPath.traverse({
                        ...
                        ImportDeclaration(astPath) {
                            ...
                            helper_2.analyzeImportUrl(self.root, sourceFilePath, scriptFiles, source, value);
                        },
                        CallExpression(astPath) {
                            ...
                            if (callee.type === 'Identifier') {
                                if (callee.name === 'require') {
                                    ...
                                    helper_2.analyzeImportUrl(self.root, sourceFilePath, scriptFiles, args[0], value);
                                }
                ...
                exit(astPath) {
                    ...
                    astPath.traverse({
                        StringLiteral(astPath) {
                            ...
                            if (extname && helper_1.REG_IMAGE.test(extname) && !helper_1.REG_URL.test(value)) {
                                ...
                                imageRelativePath = helper_1.promoteRelativePath(path.relative(sourceFilePath, sourceImagePath));
                                outputImagePath = self.getDistFilePath(sourceImagePath);
                                if (fs.existsSync(sourceImagePath)) {
                                    self.copyFileToTaro(sourceImagePath, outputImagePath);
                                    helper_1.printLog("copy" /* COPY */, '图片', self.generateShowPath(outputImagePath));
                                }
                                else if (!t.isBinaryExpression(astPath.parent) || astPath.parent.operator !== '+') {
                                    helper_1.printLog("error" /* ERROR */, '图片不存在', self.generateShowPath(sourceImagePath));
                                }
                    ...
                    if (lastImport) {
                        if (importStylePath) {
                            lastImport.insertAfter(t.importDeclaration([], t.stringLiteral(helper_1.promoteRelativePath(path.relative(sourceFilePath, importStylePath)))));
                        }
                        if (imports && imports.length) {
                            imports.forEach(({ name, ast, wxs }) => {
                                const importName = wxs ? name : helper_1.pascalCase(name);
                                ...
                                lastImport.insertAfter(template(`import ${importName} from '${helper_1.promoteRelativePath(path.relative(outputFilePath, importPath))}'`, babylon_1.default)());
                            });
                        }
                        if (depComponents && depComponents.size) {
                            depComponents.forEach(componentObj => {
                                const name = helper_1.pascalCase(componentObj.name);
                                const component = componentObj.path;
                                lastImport.insertAfter(template(`import ${name} from '${helper_1.promoteRelativePath(path.relative(sourceFilePath, component))}'`, babylon_1.default)());
                            });
                        }
                        if (isApp) {
    ...
    getApp() {
        ...
        try {
            ...
            helper_1.printLog("convert" /* CONVERT */, '入口文件', this.generateShowPath(this.entryJSPath));
            helper_1.printLog("convert" /* CONVERT */, '入口配置', this.generateShowPath(this.entryJSONPath));
            if (fs.existsSync(this.entryStylePath)) {
                this.entryStyle = String(fs.readFileSync(this.entryStylePath));
                helper_1.printLog("convert" /* CONVERT */, '入口样式', this.generateShowPath(this.entryStylePath));
            }
        }
        catch (err) {
            this.entryJSON = {};
    ...
    generateScriptFiles(files) {
        ...
        if (files.size) {
            files.forEach(file => {
                ...
                helper_1.printLog("copy" /* COPY */, 'JS 文件', this.generateShowPath(outputFilePath));
    ...
    generateEntry() {
        try {
            ...
            helper_1.printLog("generate" /* GENERATE */, '入口文件', this.generateShowPath(entryDistJSPath));
            if (this.entryStyle) {
        ...
        if (Array.isArray(list) && list.length) {
            ...
            if (icons.size > 0) {
                Array.from(icons)
                    ...
                    helper_1.printLog("copy" /* COPY */, 'TabBar 图标', this.generateShowPath(iconDistPath));
                });
            }
        }
    }
    traversePages() {
        this.pages.forEach(page => {
            ...
            try {
                ...
                const param = {};
                helper_1.printLog("convert" /* CONVERT */, '页面文件', this.generateShowPath(pageJSPath));
                if (fs.existsSync(pageConfigPath)) {
                    helper_1.printLog("convert" /* CONVERT */, '页面配置', this.generateShowPath(pageConfigPath));
                    ...
                    if (pageUsingComponnets) {
                        ...
                        Object.keys(pageUsingComponnets).forEach(component => {
                            ...
                            if (!fs.existsSync(helper_1.resolveScriptPath(componentPath))) {
                ...
                if (fs.existsSync(pageTemplPath)) {
                    helper_1.printLog("convert" /* CONVERT */, '页面模板', this.generateShowPath(pageTemplPath));
                    param.wxml = String(fs.readFileSync(pageTemplPath));
                }
                let pageStyle = null;
                if (fs.existsSync(pageStylePath)) {
                    helper_1.printLog("convert" /* CONVERT */, '页面样式', this.generateShowPath(pageStylePath));
                ...
                helper_1.printLog("generate" /* GENERATE */, '页面文件', this.generateShowPath(pageDistJSPath));
                if (pageStyle) {
                ...
                helper_1.printLog("error" /* ERROR */, '页面转换', this.generateShowPath(pageJSPath));
                console.log(err);
            }
        });
    }
    traverseComponents(components) {
        ...
        components.forEach(componentObj => {
            ...
            try {
                ...
                helper_1.printLog("convert" /* CONVERT */, '组件文件', this.generateShowPath(componentJSPath));
                if (fs.existsSync(componentConfigPath)) {
                    helper_1.printLog("convert" /* CONVERT */, '组件配置', this.generateShowPath(componentConfigPath));
                ...
                if (fs.existsSync(componentTemplPath)) {
                    helper_1.printLog("convert" /* CONVERT */, '组件模板', this.generateShowPath(componentTemplPath));
                    param.wxml = String(fs.readFileSync(componentTemplPath));
                }
                let componentStyle = null;
                if (fs.existsSync(componentStylePath)) {
                    helper_1.printLog("convert" /* CONVERT */, '组件样式', this.generateShowPath(componentStylePath));
                ...
                helper_1.printLog("generate" /* GENERATE */, '组件文件', this.generateShowPath(componentDistJSPath));
                if (componentStyle) {
                ...
                helper_1.printLog("error" /* ERROR */, '组件转换', this.generateShowPath(componentJSPath));
                console.log(err);
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            const { imports, content } = processStyleImports(style, (str, stylePath) => {
                ...
                if (path.isAbsolute(relativePath)) {
                    relativePath = helper_1.promoteRelativePath(path.relative(filePath, path.join(this.root, stylePath)));
                }
            ...
            helper_1.printLog("generate" /* GENERATE */, '样式文件', this.generateShowPath(styleDist));
            if (imports && imports.length) {
    ...
    generateConfigFiles() {
        ...
        creator.fs.commit(() => {
            ...
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(configDir, 'index.js')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(configDir, 'dev.js')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(configDir, 'prod.js')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(pkgPath));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(this.convertRoot, 'project.config.json')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(this.convertRoot, '.gitignore')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(this.convertRoot, '.editorconfig')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(this.convertRoot, '.eslintrc')));
            helper_1.printLog("generate" /* GENERATE */, '文件', this.generateShowPath(path.join(this.convertDir, 'index.html')));
            this.showLog();
        });
    }
    showLog() {
